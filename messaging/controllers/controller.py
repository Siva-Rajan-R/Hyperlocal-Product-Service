...
class ConsumersHandler:
    ...
    async def main_handler(msg:AbstractIncomingMessage):
        ...
        ic(f"Name => {domain}\nWork-For => {work_for}\nAction => {action}\nstate => {state}\nVersion => {version}")
        if f"{domain}.{work_for}"=='shops.inventory' or f"{domain}.{work_for}"=='inventory.inventory':
            await ConsumersHandler._helper(consumer_cls=InventoryConsumer,msg=msg,service_name=SERVICE_NAME.lower(),exchange_service_name=domain)
        elif f"{domain}.{work_for}"=="purchase.purchase" or f"{domain}.{work_for}"=="suppliers.purchase":
            await ConsumersHandler._helper(consumer_cls=PurchaseConsumer,msg=msg,service_name=SERVICE_NAME,exchange_service_name=domain)
        else:
            ic("Unknown event")